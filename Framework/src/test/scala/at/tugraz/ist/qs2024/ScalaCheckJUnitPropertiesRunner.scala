...
class ScalaCheckJUnitPropertiesRunner(suiteClass: java.lang.Class[Properties]) extends org.junit.runner.Runner {
  ...
  private class CustomTestCallback(notifier: RunNotifier, desc: Description) extends SchkTest.TestCallback {
    // TODO (Not for students): is it even possible to obtain the correct stack trace? ScalaCheck doesn't throw Exceptions for property failures!
  ...
  def run(notifier: RunNotifier): Unit = {
    ...
    properties.properties.foreach({
      case (desc, prop) => {
        ...
        // TODO (Not for students): is there a better way to do this? It seems that JUnit is not printing the actual name of the test case to the screen as it runs
